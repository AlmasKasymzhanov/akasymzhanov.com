...
export default async function DataPageEn() {
  const items = ARTICLES.filter((a) => DATA_SLUGS.has(a.slug)).map((a) => localizeArticle(a, L));
        ...
        <main className="w-full max-w-[1040px] mx-auto px-6 py-12 md:py-16">
          <header className="mb-10">
            ...
            <p className="text-[15px] md:text-[16px] text-[var(--color-dim)] leading-relaxed max-w-[640px]">
              Investigations, analysis, and explainers that can be verified against data. Data journalism on markets.
            </p>
          </header>
          ...
          <div className="mb-14">
            <ArticleCard a={withEngagement(lead, eng)} views={v(lead.slug)} featured headingLevel="h2" locale={L} />
          </div>
