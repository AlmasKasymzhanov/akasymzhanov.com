 ...
 * Г5 fashion share stacked bars · Г6 FBW seasonal wave · TengeJourney scrolly.
 */
...
/* ── ИНТЕРАКТИВ. «Путешествие одного тенге» ──────────────────────────
 * Scroll-telling: a tall track drives a sticky two-lane panel. Lane A (Kaspi)
 * walks the tenge out of the loop - its day-counter stops. Lane B (the Freedom
 * MODEL) keeps the tenge inside - its counter keeps ticking while the block
 * is on screen. Honest by construction: lane B carries a methodology plate. */
type JourneyStep = { title: string; detail: string };

const KASPI_STEPS: JourneyStep[] = [
  { title: "Покупка", detail: "покупатель платит в приложении" },
  { title: "Кешбэк баллами", detail: "бонусы, которые можно только потратить" },
  { title: "Выплата продавцу день в день", detail: "деньги приходят на счёт селлера" },
  { title: "Вывод в другой банк", detail: "тенге покидает контур - счётчик останавливается" },
];

const FREEDOM_STEPS: JourneyStep[] = [
  { title: "Покупка", detail: "покупатель платит в приложении" },
  { title: "Кешбэк дробными бумагами", detail: "оседает в портфеле через брокера холдинга" },
  { title: "Выручка на счёте с ежедневным доходом", detail: "остаток паркуется в инструменты денежного рынка" },
  { title: "Остаток фондирует следующую рассрочку", detail: "тенге остаётся в контуре - счётчик тикает" },
];
...
export function TengeJourney() {
  ...
  const [progress, setProgress] = useState(0); // 0..1 across the tall track
  ...
  // Freedom counter ticks while its last step is live (reduced-motion → static ∞).
  useEffect(() => {
    if (activeStep < 3) {
      setFreedomDays(0);
      return;
    ...
    if (typeof window !== "undefined" && window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      setFreedomDays(-1); // sentinel → render ∞
      return;
    }
  ...
  const kaspiDays = activeStep >= 3 ? "1 - стоп" : activeStep >= 2 ? "1" : "0";
      ...
      <div className="sticky top-[64px]">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <JourneyLane
            ...
            days={kaspiDays}
            daysLabel="дней в контуре"
          />
          <JourneyLane
            name="Контур Freedom (модель)"
            steps={FREEDOM_STEPS}
            activeStep={activeStep}
            days={freedomDisplay}
            accent
